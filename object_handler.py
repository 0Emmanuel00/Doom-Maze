...
class ObjectHandler:
    def __init__(self, game):
        ...
        self.restricted_area = {
            (x, y)
            for x in range(player_x - 3, player_x + 4)
            for y in range(player_y - 3, player_y + 4)
        }
        self.spawn_npc()
    ...
    def spawn_npc(self):
        for i in range(self.enemies):
                ...
                x, y = self.game.map.get_random_empty_position(center=self.game.player.pos, radius=35)
                while (int(x), int(y)) in self.restricted_area or (int(x), int(y)) == self.game.player.map_pos:
                    x, y = self.game.map.get_random_empty_position(center=self.game.player.pos, radius=35)
                self.add_npc(npc_type(self.game, pos=(x, y)))
